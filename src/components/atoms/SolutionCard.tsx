...
const SolutionCard: React.FC<SolutionCardProps> = ({
  ...
  return (
    <div
      onClick={onClick}
      className={`relative rounded-[1rem] overflow-hidden group feature-card hover:-translate-y-4 transition-all duration-500 shadow-2xl shadow-black/50 ${onClick ? 'cursor-pointer' : ''} ${className}`}
    >
      <div className="absolute inset-0 bg-gradient-to-br from-primary/20 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
      <div className="absolute inset-0 rounded-[1rem] border border-white/10 group-hover:border-primary/30 transition-colors duration-500 pointer-events-none z-20" />
      ...
      <div className="relative h-full w-full bg-background/40 backdrop-blur-xl overflow-hidden flex flex-col min-h-[500px]">
        ...
        <div className="p-10 pt-4 flex-1 flex flex-col justify-start space-y-4 z-10 relative">
          <div className="w-16 h-16 bg-primary rounded-2xl flex items-center justify-center text-white shadow-[0_0_30px_rgba(0,70,255,0.6)] group-hover:scale-110 transition-transform duration-500">
            {icon}
          </div>
          <Text variant="h3" className="text-4xl md:text-5xl font-bold text-white flex items-center gap-4">
